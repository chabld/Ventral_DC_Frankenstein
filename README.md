...
This is a brute fusion of multiple atlases' regions stitched up together to fit in the "ventral diencephalon" (ventral DC), a region of interest included in FreeSurfer (Fischl, 2012)'s [automated subcortical segmentation](https://surfer.nmr.mgh.harvard.edu/fswiki/SubcorticalSegmentation) (ASeg). Its purpose is to help users interpet the ventral diencephalon's anatomy in more details given it encompasses multiple nuclei, mostly as a broad reference.
...
This screenshot shows the stitched up regions overlapping with the yellow outlines of ASeg's left and right diencephalon (using FreeSurfer's freeview):

![](ventralDC_frankenstein_itsalive.png)

The remaining empty space is most likely white matter fibers extending from the brainstem and passing in between the nuclei. The cerebral peduncular tracts in particular, although defined slimmer in the JHU DTI atlas, also technically make up the anterior space of the ventral DC.

# Atlas regions used to build it

**Substantia Nigra (pars compacta and reticulata), Red Nucleus, Parabrachial Pigmented, Ventral Tegmental Area, Hypothalamus, Subthalamic Nucleus**, from the CIT68 Reinforcement Learning Atlas (Pauli et al. 2018; OSF data: https://doi.org/10.17605/OSF.IO/R2HVK), in the MNI 152 space ("CIT168_Reinf_Learn_v1.1.0/MNI152-Nonlin-Asym-2009c/CIT168toMNI152-2009c_det.nii").
...
**Medial and lateral geniculate nuclei (MGN, LGN)**, from Brainstem Navigator v1.0 (Singh et al. 2021; García-Gomar et al. 2019), which include a probablistic atlas ("2b.DiencephalicNucleiAtlas_MNI", https://www.nitrc.org/projects/brainstemnavig/)

Note: The probability threshold was set to minimal to maximize the mask sizes, as opposed to 0.35. This is justified by the Ventral DC's own rough delineation as opposed to the high resolution segmentation the Brainstem Navigator is based upon. MGN was also kept since it is documented to be part of ventral DC, even though the match with the Brainstem Navigator atlas is questionable (partial overlap).

**Left and right cerebral peduncles**, from the JHU DTI-based white-matter atlases's (Wakana et al. 2007; Hua et al. 2008; source data: https://identifiers.org/neurovault.image:1401)

# Code
The script used to merge all the regions together is stored in "code/VentralDC_Frankenstein_merger.py". It is intended to be run from a working directory that includes 3 subdirectories, i.e. "2b.diencephalicNucleiAtlas_MNI/" (including the labels_probabilistic/ subdirectory from Brainstem Navigator's OSF), "JHU/" (including the JHU atlas), and "CIT168/" including the CIT168toMNI152-2009c_det atlas (a split version, outputted using CIT168_splitter.py). The script extracts each label of interest separately, relabels them to get consistent IDs, and combines them together into the same volume. It ouputs the volume in their initial MNI152 space but also makes a version resampled to MNI305 (ASeg's space) using the transformation matrix from Buchsbaum (2026).
